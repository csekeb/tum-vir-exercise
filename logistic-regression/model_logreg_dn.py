...
class ModelLogisicRegressionMvn(LightningModule):
    def __init__(
        ...
        # if tere is a feature map add it
        if feature_map is not None:
            self.add_module("feature_map", feature_map)
        else:
            self.add_module("feature_map", nn.Sequential(nn.Identity()))
        ...
        self.weights_scale_lower   = nn.Parameter(torch.zeros((self.dim, self.dim)), requires_grad=True) if not self.is_diagonal else None
    ...
    def weights_chol(self):
        if not self.is_diagonal:
            ...
            return torch.diag(torch.exp(self.weights_scale_logdiag))

    
    def project_to_features(self, features):
        if self.is_diagonal:
            z_loc     = torch.matmul(features, self.weights_loc)
            z_scale   = torch.sqrt(torch.sum(torch.matmul(features, torch.exp(self.weights_scale_logdiag).view((-1,1)))**2, dim=-1, keepdim=True))
        else:
            L   = self.weights_chol() 
            z_loc     = torch.matmul(features, self.weights_loc)
    ...
    def forward(self, features):
        ...
        z_loc, z_scale = self.project_to_features(features)
    ...
    def loss(self, features, labels):
        ...
        z_loc, z_scale = self.project_to_features(features)      
        z_samples = Normal(loc=z_loc.squeeze(), scale=z_scale.squeeze()).rsample([self.n_samples_mc]).transpose(0,1)
        ...
        # computing KLi
        if self.is_diagonal:
            p_post  = Normal(loc=self.weights_loc.squeeze(), scale = torch.exp(self.weights_scale_logdiag))
            p_prior = Normal(loc=torch.zeros_like(self.weights_scale_logdiag),
                             scale=self.scale_prior*torch.ones_like(self.weights_scale_logdiag))
            kl_div  = torch.sum(kl_divergence(p_post, p_prior)) 
        else:
           p_post  = MultivariateNormal(loc=self.weights_loc.squeeze(),
                                        scale_tril=self.weights_chol())
